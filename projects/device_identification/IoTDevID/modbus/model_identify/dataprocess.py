...
def generate_train_test_data(file_name, train_filepath, test_filepath, ratio):
    ...
    with open(os.path.join(data_path, "modbus_model_train_ip_list.csv"), mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["train_ip"])
        # Write each row to the CSV file
        for row in train_ip_set:
            writer.writerow([row])
    with open(os.path.join(data_path, "modbus_model_test_ip_list.csv"), mode='w', newline='') as file:
        writer = csv.writer(file)
