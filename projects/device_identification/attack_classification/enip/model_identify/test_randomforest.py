...
def run():
    ...
    train_feature_list = train_data[feature]
    train_label_list = train_data["new_label_code"]

    test_data = pd.read_csv(test_filepath)
    test_data["ilrt"] = pd.cut(test_data["ilrt"], bins=bins, labels=[i for i in range(0, bins)])
    test_feature_list = test_data[feature]
    test_label_list = test_data["new_label_code"]
    ...
    score_packets(test_data, y_pred, test_label_list)
