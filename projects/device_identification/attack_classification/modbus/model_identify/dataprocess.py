...
def generate_train_test_data(df, train_filepath, test_filepath, ratio):
    ...
    with open(train_filepath, mode='w', newline='') as file:
        ...
        for row in train_list:
            ...
            if row[36] in train_label_dic:
                train_label_dic[row[36]] = train_label_dic[row[36]] +1
            else:
                train_label_dic[row[36]] = 1
    ...
    with open(test_filepath, mode='w', newline='') as file:
        ...
        for row in test_list:
            ...
            if row[36] in test_label_dic:
                test_label_dic[row[36]] = test_label_dic[row[36]] + 1
            else:
                test_label_dic[row[36]] = 1
...
def fingerprint_preprocess(df, ratio = 0.8):
    ...
    df_filtered["new_label"] = df_filtered["vendor"] + ":" + df_filtered["model"]
    df_filtered['new_label'] = pd.Categorical(df_filtered['new_label'])
    df_filtered['new_label_code'] = df_filtered['new_label'].cat.codes
    ...
    for index, raw_data in df_filtered.iterrows():
        if raw_data["new_label"] in labeled_data_dict:
            labeled_data_dict[raw_data["new_label"]].add(raw_data["IP_addr"])
        else:
            labeled_data_dict[raw_data["new_label"]] = set()
            labeled_data_dict[raw_data["new_label"]].add(raw_data["IP_addr"])
...
def train_test_split_labeled_ip(labeled_ip_data_dict, ratio):
    """ Split training and testing IP sets
    ...
    train_labeled_ip_df = pd.read_csv(os.path.join(data_path, "modbus_model_train_ip_list.csv"))
    train_labeled_ip_set = set(train_labeled_ip_df["train_ip"])

    test_labeled_ip_df = pd.read_csv(os.path.join(data_path, "modbus_model_test_ip_list.csv"))
...
code_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
project_dir = os.path.dirname(os.path.dirname(os.path.dirname(code_dir)))
data_path=os.path.join(project_dir, "data\\csv\\modbus")
